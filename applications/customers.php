...
class customers_app extends application 
{
	...
	{
		$this->application("orders", _($this->help_context = "Penjualan &Sales"));
	
		$this->add_module(_("Transactions"));
		$this->add_lapp_function(0, _("Sales &Quotation Entry ( Pemesanan Barang )"),
			"sales/sales_order_entry.php?NewQuotation=Yes", 'SA_SALESQUOTE', MENU_TRANSACTION);
		$this->add_lapp_function(0, _("Sales &Order Entry ( Order Barang ) "),
			"sales/sales_order_entry.php?NewOrder=Yes", 'SA_SALESORDER', MENU_TRANSACTION);
		$this->add_lapp_function(0, _("Direct &Delivery ( Delivery Barang Langsung )"),
			"sales/sales_order_entry.php?NewDelivery=0", 'SA_SALESDELIVERY', MENU_TRANSACTION);
		$this->add_lapp_function(0, _("Direct &Invoice ( Deliver barang dan Invoice Langsung)"),
			"sales/sales_order_entry.php?NewInvoice=0", 'SA_SALESINVOICE', MENU_TRANSACTION);
		$this->add_lapp_function(0, "","");
		$this->add_lapp_function(0, _("&Delivery Against Sales Orders ( Deliver Barang sesuai Order )"),
			"sales/inquiry/sales_orders_view.php?OutstandingOnly=1", 'SA_SALESDELIVERY', MENU_TRANSACTION);
		$this->add_lapp_function(0, _("&Invoice Against Sales Delivery ( Buka Invoice sesuai delivery )"),
		...
		$this->add_module(_("Inquiries/Tracking dan Reports"));
		...
		$this->add_lapp_function(2, _("Recurrent &Invoices (Invoice berulang)"),
