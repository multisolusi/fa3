...
if(isset($_GET['requisitionid'])) {
	...
	if(isset($_GET['complete'])) {
		if($_GET['complete'] == 'yes') {
			complete_requisition($_POST['requisitionid']);
		}
	}
}
...
$result = get_one_requisition($_POST['requisitionid']);

start_table(TABLESTYLE, "width=50%");

$th = array(_("Point of use"), _("Narrative"), _("Application Date"), _("Complete"));
...
while ($myrow = db_fetch($result)) 
{
	...
	echo "<td><a href='requisition_details.php?complete=yes&requisitionid=".$myrow['requisition_id']."'>C</a></td>\n";
...
$result = get_all_requisition_details($_POST['requisitionid']);
...
hidden('requisitionid', $_POST['requisitionid']);
